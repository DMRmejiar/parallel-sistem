...
int* best_tour;
// global variable lock critical parallel
omp_lock_t my_lock;

void swap (int *x, int *y);
int compute_cost (int *tour, int* dist, int n);
void brute_force (int *tour, int* dist, int start, int end);
void seq_brute_force (int *tour, int* dist, int start, int end);
void distribution_parallel (int *tour, int *dist, int size);
...
int main(int argc, char *argv[]) {
  ...
  time = omp_get_wtime();
  distribution_parallel(tour, dist,size);
  ...
  #pragma omp parallel
  {
    #pragma omp single
    {
      for (int start = 0; start < size; start++) {
        #pragma omp task firstprivate(start, tour) shared(size, dist, min_cost)
        {
          int * local_tour = (int *)malloc(sizeof(int) * size);
          for(int i = 0; i < size; i++){
            local_tour[i] = tour[i];
          }
          swap (&local_tour[0], &local_tour[start]);
          brute_force(local_tour, dist, 1, size - 1 );
          swap (&local_tour[0], &local_tour[start]);
        }
      }
    }
 ...
 *  \param tour pointer to swap (&tour[start], &tour[i]);the tour vector}
 *  \param dist pointer to the distance matrix
